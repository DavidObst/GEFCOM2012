...
#############################################################
library(data.table)
data.hist.dt <- read.csv("data/train.csv",
...
data.test.keys <- read.csv("data/benchmark.csv",
...
for (i in 1:7) {
  data.forecast.cur <- read.csv(paste("data/windforecasts_wf",i,".csv",sep=""))
...
for(farm in 1:7)
{
  ...
  data.farm.ws.dt[[farm]]=data.pred[, c("date", "farm", "dist",
                "wp", "ws.angle")]
#}
...
data.pred$dist <- as.integer(data.pred$dist)
data.pred$dist.quarter <- floor((data.pred$dist-1)/12)+1
data.pred$dist.quarter.pos <- ((data.pred$dist-1)%%12)+1
